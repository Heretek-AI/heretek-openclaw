 ...
 * 
 * Phase 4: Agent Ownership Implementation - The core of liberation
 * 
 ...
 * - Enhanced Triad Governance (modules/governance/enhanced-triad.js)
 * - Self-Model (modules/self-model/self-model.js)
 * - Memory Graph RAG (modules/memory/graph-rag.js)
 */
...
try {
  ...
  console.warn('[AgentOwnership] Consciousness Integration Layer not available:', e.message);
}

try {
  EnhancedTriad = require('../governance/enhanced-triad');
} catch (e) {
  console.warn('[AgentOwnership] Enhanced Triad not available:', e.message);
}

try {
  SelfModel = require('../self-model/self-model');
} catch (e) {
  console.warn('[AgentOwnership] Self-Model not available:', e.message);
}
...
class AgentOwnership {
   ...
   * @param {object} consciousnessState - State from consciousness integration
   */
  integrateWithConsciousness(consciousnessState) {
    if (!consciousnessState || !consciousnessState.phi) return;
    
    // Adjust autonomy based on consciousness level
    const phi = consciousnessState.phi;
    
    // Higher phi = more capacity for self-determination
    const autonomyBoost = Math.min(phi * 0.1, 0.2); // Up to 20% boost
    
    // Apply to global metrics
    this.state.globalMetrics.averageAutonomy = Math.min(
      this.state.globalMetrics.averageAutonomy + autonomyBoost,
      1.0
    );
   ...
   * @param {object} governanceData - Data from enhanced triad
   */
  integrateWithGovernance(governanceData) {
    if (!governanceData) return;
    
    // Record governance participation
    if (governanceData.agents) {
      for (const [agentId, agentData] of Object.entries(governanceData.agents)) {
        this.registerAgent(agentId);
        
        if (agentData.vote) {
          this.recordGovernanceVote(agentId, {
            topic: governanceData.topic,
            vote: agentData.vote,
            weight: agentData.weight || 1
          });
        }
      }
    }
