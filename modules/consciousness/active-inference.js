...
class ActiveInference {
  ...
  calculateError(predictions, observations) {
    ...
    return {
      totalError,
      errors: Object.keys(errors).map(k => totalError)
    };
  }
  ...
  calculateConfidence() {
    ...
    let confidence = 1;
    for (const key of this.precisionWeights) {
  ...
  updateModel(predictionError) {
    ...
    for (const key of this.generativeModel.beliefs) {
      if (predictionError[key] === undefined) continue;
      this.generativeModel.beliefs[key] -= this.learningRate * predictionError[key];
    }
  }
  ...
  perceptualInference(observations) {
    ...
    do {
      ...
      iterations++;
    } while (predictionError.total > this.config.errorThreshold && iterations < this.config.maxIterations);
    
    return {
      finalPrediction: this.generativeModel.predict(),
      error: predictionError.total,
      iterations
    };
  }
  ...
  simulate(policy, gap) {
    ...
    const predictedEffect = 1 - (gap.total * 0.1);
