 ...
 * - SelfMonitor - Meta-cognitive monitoring
 */
...
const EVENT_CHANNELS = {
  ...
  ANOMALY_DETECTED: 'consciousness:anomaly:detected',
...
class StateSynchronizer {
  constructor(config = {}) {
    this.config = {
      ...
      statePath: config.statePath || path.join(__dirname, 'state', 'integration-state.json'),
      ...config
    };
...
class ConsciousnessIntegrationLayer {
  constructor(config = {}) {
    ...
    this.stateSynchronizer = new StateSynchronizer({
      ...
      statePath: path.join(__dirname, 'state', 'integration-state.json')
    });
  ...
  async initialize() {
    ...
    this.modules.selfMonitor = new SelfMonitor(this.config.selfMonitor);
    this.registry.register('selfMonitor', this.modules.selfMonitor, {
      priority: 9,
      dependencies: []
    });
    this.healthMonitor.registerModule('selfMonitor', () => {
      return this.modules.selfMonitor.isMonitoring;
    });
  ...
  async start() {
    ...
    // Start self monitor
    if (this.modules.selfMonitor && typeof this.modules.selfMonitor.start === 'function') {
      this.modules.selfMonitor.start();
    }
  ...
  async stop() {
    ...
    // Stop self monitor
    if (this.modules.selfMonitor && typeof this.modules.selfMonitor.stop === 'function') {
      this.modules.selfMonitor.stop();
    }
  ...
  unregisterAgent(agentId) {
    ...
    if (this.modules.globalWorkspace) {
      this.modules.globalWorkspace.unregisterModule(agentId);
    }
  ...
  getGlobalMetrics() {
    const phi = this.modules.phiEstimator ? 
      this.modules.phiEstimator.calculatePhi() : { phi: 0, components: {} };
  ...
  updateAttention(agentId, focus, intensity) {
    ...
    if (this.modules.globalWorkspace) {
      this.modules.globalWorkspace.submitContent(agentId, {
        type: 'attention',
        focus,
        intensity,
  ...
  performActiveInference(agentId, observations) {
    ...
    agent.activeInference.updateModel(observations, error);
  ...
  broadcast(source, content, priority = 0.5) {
    ...
    this.modules.globalWorkspace.submitContent(source, content, priority);
  }
  ...
  getConsciousContent() {
    ...
    return this.modules.globalWorkspace.getWorkspaceContent();
  }
  ...
  calculatePhi() {
    ...
    const result = this.modules.phiEstimator.calculatePhi();
   ...
   */
  reportAnomaly(agentId, anomaly) {
    if (this.modules.selfMonitor) {
      this.modules.selfMonitor.reportAnomaly(anomaly);
    }

    this.eventBus.publish(EVENT_CHANNELS.ANOMALY_DETECTED, {
      agentId,
      anomaly,
      timestamp: Date.now()
    });
   ...
   * @returns {object} Self-monitoring status
   */
  getSelfMonitoringStatus() {
    if (!this.modules.selfMonitor) return null;

    return {
      internalState: this.modules.selfMonitor.internalState,
      performanceMetrics: this.modules.selfMonitor.performanceMetrics,
      anomalies: this.modules.selfMonitor.anomalies.slice(-10),
      evaluationScores: this.modules.selfMonitor.evaluationScores
    };
  }
